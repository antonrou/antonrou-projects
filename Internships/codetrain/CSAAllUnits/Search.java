...
class Search {
  ...
  public static int binarySearchIntegers(ArrayList<Integer> list, Integer searchTerm) {
    int start = 0;
    int end = list.size() - 1;
    while (start <= end) {
      int mid = (start + end) / 2;
      if (searchTerm == list.get(mid)) {
        return mid;
      } else if (searchTerm > list.get(mid)) {
        start = mid + 1;
      } else {
        end = mid - 1;
      }
    ...
    return -1;
  }// binarySearchIntegers

  public static int binarySearchDoubles(ArrayList<Double> list, Double searchTerm) {
    int start = 0;
    int end = list.size() - 1;
    while (start <= end) {
      int mid = (start + end) / 2;
